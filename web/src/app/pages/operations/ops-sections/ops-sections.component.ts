...
import { OpsSectionsTableColumn } from 'src/app/shared/utility/table';
...
@Component({
  selector: 'app-ops-sections',
  templateUrl: './ops-sections.component.html',
  styleUrls: ['./ops-sections.component.scss'],
  host: {
    class: "page-component"
  }
})
export class OpsSectionsComponent {
  currentOperatorCode:string;
  error:string;
  dataSource = new MatTableDataSource<OpsSectionsTableColumn>();
  columnDefs = [];
  displayedColumns = [];
  isLoading = false;
  isProcessing = false;
  ...
  showSelectSchoolYearLevelDialog() {
    ...
    dialogRef.afterClosed().subscribe((res:SchoolYearLevels)=> {
      ...
      if(res) {
        ...
        this._location.go("/ops/sections/find/" + this.selectedSchool?.schoolCode + "/sylvl/" + this.selectedSchoolYearLevel?.schoolYearLevelCode);
        this.getSectionsPaginated();
      }
    })
  }
  ...
  controlMenuItemSelected(type: "edit" | "delete", data: OpsSectionsTableColumn) {
    console.log(type, data);
    if(type === "edit") {
      const dialogRef = this.dialog.open(OpsSectionFormComponent, {
        maxWidth: '720px',
        width: '720px',
        disableClose: true,
